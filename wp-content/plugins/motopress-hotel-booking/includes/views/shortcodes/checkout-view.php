...
?>
	...
	?>
		<form class="mphb_sc_checkout-form" method="POST" action="<?php echo esc_url($actionUrl); ?>">
			...
			<input type="hidden" name="<?php echo esc_attr(\MPHB\Shortcodes\CheckoutShortcode::BOOKING_CID_NAME); ?>" value="<?php echo esc_attr($checkoutId); ?>" />
			<input type="hidden" name="mphb_check_in_date" value="<?php echo $booking->getCheckInDate()->format(MPHB()->settings()->dateTime()->getDateTransferFormat()); ?>" />
			<input type="hidden" name="mphb_check_out_date" value="<?php echo $booking->getCheckOutDate()->format(MPHB()->settings()->dateTime()->getDateTransferFormat()); ?>" />
