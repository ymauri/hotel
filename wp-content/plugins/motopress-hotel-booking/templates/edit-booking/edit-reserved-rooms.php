...
<form class="mphb-reserve-rooms" action="<?php echo esc_attr($actionUrl); ?>" method="POST">
    ...
    <h2>
        <?php _e('Edit Accommodations', 'motopress-hotel-booking'); ?>
        <button id="mphb-add-room-button" class="add-new-h2"><?php _e('Add Accommodation', 'motopress-hotel-booking'); ?></button>
    </h2>

    <p class="description"><?php _e('Add, remove or replace accommodations in the original booking.', 'motopress-hotel-booking'); ?></p>

    <table class="widefat striped fixed mphb-reserve-rooms-table">
        <thead>
            <tr>
                <th class="row-title column-room-type"><?php _e('Accommodation Type', 'motopress-hotel-booking'); ?></th>
                <th class="row-title column-room"><?php _e('Accommodation', 'motopress-hotel-booking'); ?></th>
                <th class="row-title column-status"><?php _e('Status', 'motopress-hotel-booking'); ?></th>
                <th class="row-title column-actions"><?php _e('Actions', 'motopress-hotel-booking'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($reservedRooms as $roomId => $room) {
                ...
                <tr class="mphb-reserve-room <?php echo $isAvailable ? 'mphb-available-room' : 'mphb-unavailable-room'; ?>" data-room-id="<?php echo esc_attr($roomId); ?>" data-room-type-id="<?php echo esc_attr($room['room_type_id']); ?>">
                    ...
                    <td class="column-status">
                        ...
                        <?php } ?>
                        <span><?php $isAvailable ? _e('Available', 'motopress-hotel-booking') : _e('Not Available', 'motopress-hotel-booking'); ?></span>
                    </td>
                    <td class="column-actions">
                        <button class="button mphb-remove-room-button"><?php _e('Remove', 'motopress-hotel-booking'); ?></button>
                        <button class="button mphb-replace-room-button"><?php _e('Replace', 'motopress-hotel-booking'); ?></button>
                    </td>
                </tr>
    ...
    <p class="mphb-submit-button-wrapper">
        <input type="submit" name="edit-booking" class="button button-primary" value="<?php _e('Continue', 'motopress-hotel-booking'); ?>" <?php disabled(!$isValid); ?>>
    </p>
</form>
<?php
