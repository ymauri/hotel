...
class CheckoutView
{
    ...
    public static function renderBookedDetails($booking, $rooms)
    {
        ...
        <section class="mphb-booking-details mphb-checkout-section">
            <h3 class="mphb-booking-details-title">
                <?php _e('Original Booking Details', 'motopress-hotel-booking'); ?>
            </h3>
    ...
    public static function renderGuests($reservedRoom, $roomIndex, $roomType)
    {
        ...
        if ($adultsAllowed) {
            ...
            <p class="mphb-adults-chooser">
                <label>
                    <?php $childrenAllowed ? _e('Adults', 'motopress-hotel-booking') : _e('Guests', 'motopress-hotel-booking'); ?>
                </label>
                <input type="text" name="edit-<?php echo uniqid(); ?>-adults" value="<?php echo esc_attr($adults); ?>" readonly="readonly">
            </p>
            <?php
        }
        ...
        if ($childrenAllowed && $roomType->getChildrenCapacity() > 0) {
            ...
            <p class="mphb-children-chooser">
                ...
                <input type="text" name="edit-<?php echo uniqid(); ?>-children" value="<?php echo esc_attr($children); ?>" readonly="readonly">
            </p>
            <?php
        }
        ...
        <p class="mphb-guest-name-wrapper">
            ...
            <input type="text" name="edit-<?php echo uniqid(); ?>-guest-name" value="<?php echo esc_attr($guestName); ?>" readonly="readonly">
        </p>
        <?php
    }
    ...
    public static function renderRate($reservedRoom, $roomIndex, $roomType, $booking)
    {
        ...
        <section class="mphb-rate-chooser mphb-checkout-item-section">
            <h4 class="mphb-room-rate-chooser-title">
                ...
                printf(__('Rate: %s', 'motopress-hotel-booking'), $rateInfo);
                ?>
            </h4>
        </section>
        <?php
    ...
    public static function renderServices($reservedRoom)
    {
        ...
        <section class="mphb-services-details mphb-checkout-item-section">
            <h4 class="mphb-services-details-title">
                <?php _e('Additional Services', 'motopress-hotel-booking'); ?>
            </h4>
            ...
            <ul class="mphb_sc_checkout-services-list mphb_checkout-services-list">
                ...
                foreach ($services as $service) {
                    ...
                    <li>
                        <label>
                            <input type="checkbox" name="edit-<?php echo uniqid(); ?>[service_id]" class="mphb_sc_checkout-service mphb_checkout-service" value="" checked="checked" disabled="disabled">
                            <?php echo $translatedService->getTitle(); ?>

                            <em>(<?php echo $service->getPriceWithConditions(false); ?>)</em>

                            <?php if ($service->isPayPerAdult()) { ?>
                                <?php printf(_n('x %d guest', 'x %d guests', $service->getAdults(), 'motopress-hotel-booking'), $service->getAdults()); ?>
                            <?php } ?>

                            <?php if ($service->isFlexiblePay()) { ?>
                                <?php printf(_n('x %d time', 'x %d times', $service->getQuantity(), 'motopress-hotel-booking'), $service->getQuantity()); ?>
                            <?php } ?>
                        </label>
                    </li>
                    <?php
