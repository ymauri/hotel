...
<form class="" action="<?php echo esc_attr($actionUrl); ?>" method="POST">
    ...
    <h2><?php _e('Choose how to associate data', 'motopress-hotel-booking'); ?></h2>
	<p class="description"><?php _e('Use Source Accommodation to assign pre-filled booking information available in the original booking, e.g., full guest name, selected rate, services, etc.', 'motopress-hotel-booking'); ?></p>

    <table class="widefat striped fixed">
        <thead>
            <tr>
                <th class="row-title column-room"><?php _e('Source accommodation', 'motopress-hotel-booking'); ?></th>
                <th class="row-title column-transition">&nbsp;</th>
                <th class="row-title column-preset-room"><?php _e('Target accommodation', 'motopress-hotel-booking'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
                ...
                <tr>
                    <td class="column-preset-room">
                        ...
                        <select name="map_rooms[<?php echo $index; ?>][reserved_room_id]">
                            <?php echo mphb_tmpl_render_select_options($copyFrom, $reservedRoomId); ?>
                        </select>
                    </td>
    ...
    <p class="mphb-submit-button-wrapper">
        <input type="submit" name="edit-booking" class="button button-primary" value="<?php _e('Continue', 'motopress-hotel-booking'); ?>">
    </p>
</form>
<?php
